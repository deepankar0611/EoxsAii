...
const Chat = () => {
  ...
  const handleSendMessage = async (content: string, files?: File[], isDocumentQuery?: boolean) => {
    ...
    try {
      ...
      if (isDocumentQuery && files && files.length > 0) {
        ...
        console.log("🚀 [FRONTEND] Calling /api/messages with data:", {
          threadId: selectedThreadId,
        ...
        if (!response.ok) {
          const errorData = await response.text();
          console.error("❌ [FRONTEND] Message request failed:", {
        ...
        console.log("✅ [FRONTEND] /api/messages call successful");
        aiResponseData = await response.json();
        console.log("📄 [FRONTEND] AI response data:", {
          hasAnswer: !!aiResponseData.answer,
          hasContext: !!aiResponseData.context,
          hasResponseId: !!aiResponseData.responseId,
          answerLength: aiResponseData.answer?.length || 0
        });
      }
      ...
      } else if (aiResponseData?.answer) {
        // Handle direct response format from API
        aiContent = aiResponseData.answer;
        console.log("Found answer in direct response:", aiContent);
      } else if (aiResponseData?.content) {
        // Handle direct content format
        aiContent = aiResponseData.content;
        console.log("Found content in direct response:", aiContent);
      } else {
      ...
      console.log("✅ [FRONTEND] AI message added to UI");

      // Update message IDs with real database IDs if available (without refetching all messages)
      if (selectedThreadId && aiResponseData?.data?.id) {
        setMessages(prev => prev.map(msg => 
          msg.id === aiMessage.id 
            ? { ...msg, id: aiResponseData.data.id }
            : msg
        ));
      }
  ...
  const updateAllThreadTitles = useCallback(async (userId: string) => {
    try {
