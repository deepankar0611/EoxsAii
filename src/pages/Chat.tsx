...
const Chat = () => {
  ...
  const handleSendMessage = async (content: string, files?: File[], isDocumentQuery?: boolean) => {
    ...
    try {
      ...
      console.log("✅ [FRONTEND] AI message added to UI, now fetching fresh messages from database");

      // Fetch fresh messages from database to get real IDs and ensure persistence
      if (selectedThreadId) {
        console.log("🔄 [FRONTEND] Fetching fresh messages from database for thread:", selectedThreadId);
