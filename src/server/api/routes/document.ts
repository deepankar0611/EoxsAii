...
async function parseDocument(file: multer.File): Promise<string> {
  if (file.mimetype === "application/pdf") {
    ...
    if (typeof pdfResult === 'string') {
      ...
      return String((pdfResult as any).text);
    } else {
      throw new Error('Unexpected PDF parse result');
    }
...
Answer:`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a technical document analyzer that focuses on explaining code examples and implementations. When you see code, explain what it does, how it works, and its key features. Be specific about the actual content shown in the documents. Provide comprehensive, detailed analysis that covers all aspects of the code and concepts presented.",
        },
        {
          role: "user",
          content: prompt,
