...
router.get('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await withDatabase(async () => {
    ...
    const [threads, total] = await Promise.all([
      (Thread as any).find({ userId }).sort({ [sort]: order }).skip(skip).limit(limit).lean(),
      (Thread as any).countDocuments({ userId })
    ]);
...
router.post('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  const thread = await withDatabase(async () => {
    ...
    const newThread = await (Thread as any).create({
      userId,
...
router.get('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  const thread = await withDatabase(async () => {
    const result = await (Thread as any).findById(id).lean();
...
router.delete('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  await withDatabase(async () => {
    ...
    await (Message as any).deleteMany({ threadId: thread._id });
    // Delete the thread
...
router.get('/:id/messages', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await withDatabase(async () => {
    ...
    const [messages, total] = await Promise.all([
      (Message as any).find({ threadId: id }).sort({ createdAt: sortDirection }).skip(skip).limit(limit).lean(),
      (Message as any).countDocuments({ threadId: id })
    ]);
    ...
    const formattedMessages = messages.map(msg => ({
      ...
      role: msg.sender === 'system' ? 'ai' : 'user',
...
router.get('/admin/all', requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await withDatabase(async () => {
    ...
    const [threads, total] = await Promise.all([
      (Thread as any).find(filter).sort({ [sort]: order }).skip(skip).limit(limit).lean(),
      (Thread as any).countDocuments(filter)
    ]);
...
router.post('/update-titles', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await withDatabase(async () => {
    ...
    for (const thread of threads) {
      ...
      const firstMessage = await (Message as any).findOne({ 
        threadId: thread._id 
      }).sort({ createdAt: 1 });
